...
{
  return (struct sockaddr*)((void*)(addr));
}

const struct sockaddr* sockaddr_cast(
    const struct sockaddr_in* addr)
{
  return (const struct sockaddr*)((const void*)(addr));
}

const struct sockaddr_in* sockaddr_in_cast(
    const struct sockaddr* addr)
{
  return (const struct sockaddr_in*)((const void*)(addr));
}

const struct sockaddr_in6* sockaddr_in6_cast(
    const struct sockaddr* addr)
{
  return (const struct sockaddr_in6*)((const void*)(addr));
}

int createNonblockingOrDie(sa_family_t family)
{
//#if VALGRIND
  // 得到指定地址族＋指定传输类型＋指定协议类型的套接字
  int sockfd = ::socket(
          family, 
          SOCK_STREAM, 
          IPPROTO_TCP);
